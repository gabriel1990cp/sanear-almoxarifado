...
class EstoqueSaida_model extends CI_Model
{
    #CADASTRAR ESTOQUE ENTRADA
    function inserirSaida($data)
    {
        $this->db->insert('saida_estoque', $data);
    ...
    function verificaEquipeCadastrada($equipe)
    {
        $this->db->select('*');
        $this->db->from("saida_estoque");
        $this->db->where("equipe_est_saida",$equipe);
        $this->db->where("status_est_saida",'aberto');
